...
# Use a jinja template to create the Anaconda environment files
env = Environment(loader=FileSystemLoader("templates"), trim_blocks=True,  lstrip_blocks=True)
template = env.get_template("environment.yml.j2")

for test_variables in test_versions:
    parsed_template = template.render(test_variables)
 
    with open("environment-tf%s-py%s-%s.yml" % (test_variables["tensorflow_version"],
    test_variables["python_version"], test_variables["math_library"]), "w") as fh:
        fh.write(parsed_template)


# Use a jinja template to create the shell script to run the tests
template = env.get_template("run_local_tests.sh.j2")
parsed_template = template.render(test_versions=test_versions)

with open("run_local_tests.sh", "w") as fh:
    fh.write(parsed_template)
...
for test_variables in test_versions:
    ...
    with open("miniconda-tf%s-py%s-%s.def" % (test_variables["tensorflow_version"],
    test_variables["python_version"], test_variables["math_library"]), "w") as fh:
        fh.write(parsed_template)
